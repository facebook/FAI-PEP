...
class BuildProgramTest(unittest.TestCase):
    def setUp(self):
        ...
        self.actual_file = os.path.join(gettempdir(), "aibenchtest2",
                                        "program")
    ...
    def testBuckBuild(self):
        with patch("utils.subprocess_with_logger.processRun",
    ...
    def testisBuildSuccessful(self):
        self.assertFalse(_isBuildSuccessful(self.fake_file, "ios",
                                            "buck build aibench:run"))
        self.assertTrue(_isBuildSuccessful(self.actual_file, "oculus",
                        "buck build aibench:run"))
