...
class RunBenchTest(unittest.TestCase):
    ...
    def _check_repo_driver_call_params(self, cmd):
        ...
        for i in range(2, len(cmds)):
            if cmds[i].startswith("--") and i + 1 < len(cmds) and \
                    not cmds[i + 1].startswith("--"):
                self.assertEqual(cmds[i + 1], self.params[cmds[i]])
            elif cmds[i].startswith("--") and (i + 1 == len(cmds) or
                    cmds[i + 1].startswith("--")):
    ...
    def check_config_file_content(self, args):
        config_file = os.path.join(BENCHMARK_DIR,
                                    "test/test_home/.aibench/git/config.txt")
        with open(config_file) as f:
            config_content = json.load(f)
            for key in ["--remote_reporter", "--remote_access_token",
                        "--root_model_dir", "--screen_reporter"]:
    ...
    def test_existing_config(self):
        ...
        with patch("os.system", side_effect=self._check_repo_driver_call_params) \
                as repo_driver,\
            patch("utils.arg_parse.parseKnown", return_value=(argparse.Namespace(
    ...
    def test_new_config(self):
        ...
        with patch("os.system", return_value=0) as repo_driver, \
                patch('run_bench.RunBench._inputOneArg',
                side_effect=self._mock_input_arg),\
                patch("argparse.ArgumentParser.parse_known_args",
                return_value=(argparse.Namespace(
