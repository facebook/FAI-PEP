...
class RepoDriverTest(unittest.TestCase):
    ...
    def test_run(self):
        ...
        with patch("repo_driver.parseKnown",
                return_value=(argparse.Namespace(), [])),\
                patch("repo_driver.getArgs",
                return_value=self.args),\
                patch("repo_driver.getRepo", return_value=GitRepo("/test/pytorch")),\
                patch("repo_driver.ExecutablesBuilder._setupRepoStep",
    ...
    def test_buildProgram(self):
        with patch("repo_driver.parseKnown",
                ...
                patch("repo_driver.getArgs",
                return_value=self.args),\
                patch("repo_driver.getRepo",
                return_value=GitRepo("/test/pytorch")),\
                patch("repo_driver.ExecutablesBuilder._buildProgramPlatform",
                return_value=None),\
                patch("os.listdir",
                return_value=os.path.join(BENCHMARK_DIR, "test/test_config")):
            app = ExecutablesBuilder(GitRepo("/test/pytorch"),
                                     threading.Lock(), deque())
    ...
    def test_getControlCommit(self):
        with patch("repo_driver.parseKnown",
                ...
                patch("repo_driver.getArgs",
                return_value=self.args),\
                patch("repos.git.GitRepo.getCommitsInRange",
                return_value="1:2\n3:4"):
            app = ExecutablesBuilder(GitRepo("/test/pytorch"),
                                     threading.Lock(), deque())
